...
vb = r/4*[-1/(l+w),1/(l+w),1/(l+w),-1/(l+w);1,1,1,1;-1,1,-1,1]*dtheta'; % 轮子速度
if vb(1)==0
    dqb = [0;vb(2);vb(3)];
else
    dqb = [vb(1);(vb(2)*sin(vb(1))+vb(3)*(cos(vb(1))-1))/vb(1);(vb(3)*sin(vb(1))-vb(2)*(cos(vb(1)-1)))/vb(1)];
end
dq = [1,0,0;0,cos(current_conf(1)),-sin(current_conf(1));0,sin(current_conf(1)),cos(current_conf(1))]*dqb; % 机器人速度
