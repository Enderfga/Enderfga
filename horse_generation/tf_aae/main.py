...
from src.dataflow.horse import HorseData
...
DATA_PATH = './data/cifar10_horse/train'
SAVE_PATH = './data/out/aae_horse/'
Gene_SAVE_PATH = './data/out/aae_horse/generate/'
...
def get_args():
    ...
    parser.add_argument('--bsize', type=int, default=128,
                        help='Batch size')
    parser.add_argument('--maxepoch', type=int, default=10001,
...
def semisupervised_train():
    ...
    train_model = AAE(
        n_code=FLAGS.ncode, wd=0, n_class=1, add_noise=FLAGS.noise,
    ...
    cls_valid_model = AAE(n_code=FLAGS.ncode, n_class=1)
...
def supervised_train():
    ...
    model = AAE(n_code=FLAGS.ncode, wd=0, n_class=1, 
    ...
    valid_model = AAE(n_code=FLAGS.ncode, use_supervise=True, n_class=1)
    ...
    generator = Generator(
        generate_model=valid_model, save_path=SAVE_PATH, n_labels=1)
...
def train():
    ...
    model = AAE(n_code=FLAGS.ncode, wd=0, n_class=1, 
                add_noise=FLAGS.noise, enc_weight=FLAGS.encw, 
                gen_weight=FLAGS.genw, dis_weight=FLAGS.disw)
    model.create_train_model()
    ...
    valid_model = AAE(n_code=FLAGS.ncode, n_class=1)
    ...
    generator = Generator(generate_model=valid_model, save_path=SAVE_PATH,
                          distr_type=FLAGS.dist_type, n_labels=1)
...
def generate():
    ...
    generate_model = AAE(n_code=FLAGS.ncode, n_class=1)
    ...
    generator = Generator(generate_model=generate_model, save_path=Gene_SAVE_PATH,
                          distr_type=FLAGS.dist_type, n_labels=1, use_label=FLAGS.label)
    ...
    with tf.Session(config=sessconfig) as sess:
        ...
        if FLAGS.supervise:
            ...
            for i in range(1000):
                generator.generate_samples(sess, plot_size=plot_size, file_id=i)
...
def visualize():
    ...
    model = AAE(n_code=FLAGS.ncode, use_label=FLAGS.label, n_class=1)
    model.create_train_model()
